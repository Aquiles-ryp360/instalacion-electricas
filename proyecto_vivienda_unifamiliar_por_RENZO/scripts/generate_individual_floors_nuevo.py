...
DGE_SYMBOL_PATH = Path(__file__).resolve().parents[2] / "herramientas" / "simbologia-ia" / "simbologia_normativa_dge.json"
DGE_SYMBOLS = {}
...
def main():
    ...
    json_path = project_root / "datos_diseno" / "modelo_electrico" / "instalaciones_electricas_nueva.json"
    ...
    for i, floor in enumerate(data["floors"]):
        ...
        title_text = f"IE-01 PLANO ELECTRICO - {floor_name.upper()}"
        ...
        # Save DXF
        out_dxf_path = project_root / "planos" / "entregables" / "dxf" / f"plano_electrico_piso{floor_num}.dxf"
        doc.saveas(out_dxf_path)
        print(f"DXF generado: {out_dxf_path}")
        
        # Save PDF
        out_pdf_path = project_root / "planos" / "entregables" / "pdf" / f"plano_electrico_piso{floor_num}.pdf"
