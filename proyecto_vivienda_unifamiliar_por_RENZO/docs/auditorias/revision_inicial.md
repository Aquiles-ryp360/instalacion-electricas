...
- Directorios principales:
  - `Avanze-Proyecto-Aquiles/`: Contiene el avance y material del alumno Aquiles Taylor. Incluye notas normativas, borradores de informes y su propio espacio CAD (`trabajo-cad-casa/`).
  ...
  - `proyecto_vivienda_unifamiliar_por_RENZO/`: **Carpeta de trabajo exclusivo para este proyecto**, que contiene el informe en LaTeX, planos base y entregables eléctricos.

#### Carpeta del Motor CAD (`/herramientas/ia-cad-casas/`)
...
#### Carpeta del Proyecto de Renzo (`/proyecto_vivienda_unifamiliar_por_RENZO/`)
...
- **Planos Base JPG:** Las imágenes de los planos base se encuentran duplicadas en `proyecto_vivienda_unifamiliar_por_RENZO/planos/` (`primer-piso.jpg`...) y en `latex/planos/` (`piso-1.jpg`...). Ambas corresponden a la misma arquitectura de 3 niveles de la vivienda unifamiliar.
- **Códigos de Renderizado:** Existen dos lógicas de dibujo: la basada en archivos JSON (`ia-cad-casas`) y la basada en coordenadas fijas en Python (`draw_planos.py`).
...
2.  **Incompatibilidad de Nombres y Membretes:** El proyecto guía de la carpeta `/Avanze-Proyecto-Aquiles/` tiene referencias al alumno "Aquiles Taylor", mientras que este proyecto debe estar personalizado en su totalidad para **"Renzo Gabriel Mamani Galindo"**.
...
- El informe LaTeX de `proyecto_vivienda_unifamiliar_por_RENZO` se mantiene como la base del entregable final de 72 páginas, por lo que toda mejora debe alimentar directamente sus figuras y cuadros.
