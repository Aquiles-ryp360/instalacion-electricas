...
def main():
    ...
    for i, floor in enumerate(data["floors"]):
        ...
        # Save DXF
        out_dxf_path = project_root / "06_planos" / "entregables" / "dxf" / f"plano_electrico_piso{floor_num}.dxf"
        out_dxf_path.parent.mkdir(parents=True, exist_ok=True)
        doc.saveas(out_dxf_path)
        print(f"DXF generado: {out_dxf_path}")
        
        # Save PDF
        out_pdf_path = project_root / "06_planos" / "entregables" / "pdf" / f"plano_electrico_piso{floor_num}.pdf"
