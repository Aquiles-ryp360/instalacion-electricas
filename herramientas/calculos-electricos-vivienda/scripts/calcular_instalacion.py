...
def calculate_circuit(circ, params):
    ...
    requires_diff = circ["id"] in ["C2", "C3", "C5", "C6", "C7", "C8"]
...
def generate_markdown_report(results, output_filepath):
    lines = [
        "# Reporte Tecnico de Calculos Electricos - Caso Aquiles",
    ...
    lines.extend(
        [
            ...
            "## 3. Notas",
            "",
            "- El escenario con cocina electrica es conservador y no confirma que Aquiles instalara cocina electrica.",
...
def parse_args():
    ...
    parser.add_argument(
        "--input",
        default=os.path.join(project_root, "data", "proyecto_aquiles_base.json"),
...
def main():
    ...
    output_json = os.path.join(args.output_dir, "resultados_aquiles.json")
