#!/usr/bin/env python3
"""Genera la entrega academica final de metrados y presupuesto de Aquiles."""
...
def escribir_excel_final(
    ...
    datos_portada = [
        ...
        ("PROYECTO", "Proyecto de instalaciones electricas de vivienda"),
    ...
    portada["A1"] = "PRESUPUESTO FINAL DE INSTALACIONES ELECTRICAS - AQUILES"
...
def escribir_resumen(
    ...
    lineas = [
        "# Resumen del presupuesto final de Aquiles", "",
...
def escribir_latex_final(
    ...
    fragmento = output_dir / "presupuesto_final_aquiles.tex"
    wrapper = output_dir / "presupuesto_final_aquiles_documento.tex"
    ...
    contenido = (
        "% Fragmento listo para: \\input{Avanze-Proyecto-Aquiles/cotizacion-materiales-final/presupuesto_final_aquiles.tex}\n"
        ...
        "Antes de comprar deben confirmarse el alimentador de T2, las salidas C6, las protecciones diferenciales y todos los componentes del SPAT.\n"
    )
    ...
    pdf = output_dir / "presupuesto_final_aquiles.pdf"
    log = output_dir / "presupuesto_final_aquiles.latex.log"
    if shutil.which("pdflatex"):
        ...
        generado = output_dir / "presupuesto_final_aquiles_documento.pdf"
        if correcto and generado.exists():
            generado.replace(pdf)
    return {"latex": fragmento, "latex_documento": wrapper, "pdf": pdf, "latex_log": log}


def escribir_readme(path: Path, resumen: List[Dict[str, Any]], recomendado: List[Dict[str, Any]]) -> None:
...
def generar_entrega_final(
    ...
    paths["excel_final"] = output_dir / "presupuesto_final_aquiles.xlsx"
    ...
    escribir_readme(paths["readme"], resumen, recomendado)
    (output_dir / "bom_final_aquiles.json").write_text(
    ...
    (output_dir / "presupuesto_final_aquiles.json").write_text(
