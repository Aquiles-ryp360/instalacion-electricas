...
Uso:
  python3 generar_unifilar.py --resultados calculos/output/resultados_aquiles.json --output diagrama_unifilar.dxf
  python3 generar_unifilar.py --resultados calculos/output/resultados_aquiles.json --output diagrama_unifilar.pdf
"""
...
def generar_unifilar(resultados, output_path):
    ...
    for c in circuitos:
        ...
        has_ground = c["id"] in ["C2", "C3", "C5", "C6", "C7", "C8"]
...
def parse_args():
    ...
    parser.add_argument("--resultados", required=True,
                       help="JSON de resultados (resultados_aquiles.json)")
