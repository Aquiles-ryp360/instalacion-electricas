...
def main():
    ...
    json_path = root / "proyecto_vivienda_unifamiliar_por_RENZO" / "electricos" / "data" / "instalaciones_electricas_nueva.json"
    ...
    for i, floor in enumerate(data["floors"]):
        ...
        base_dxf_path = root / floor["base_dxf"]
        ...
        # Save DXF
        out_dxf_path = root / "planos" / "salida-dxf" / f"plano_electrico_piso{floor_num}.dxf"
        doc.saveas(out_dxf_path)
        print(f"DXF generado: {out_dxf_path}")
        
        # Save PDF
        out_pdf_path = root / "planos" / "salida-pdf" / f"plano_electrico_piso{floor_num}.pdf"
