...
def sym_lum(msp, x, y, circuit):
    ...
    circle(msp, (x, y), 0.16, "ELEC_LUMINARIAS", color)
    line(msp, (x - 0.11, y - 0.11), (x + 0.11, y + 0.11), "ELEC_LUMINARIAS", color)
    line(msp, (x - 0.11, y + 0.11), (x + 0.11, y - 0.11), "ELEC_LUMINARIAS", color)


def sym_switch(msp, x, y, kind, circuit):
    color = CIRCUIT_COLORS.get(circuit, COLORS["orange"])
    circle(msp, (x, y), 0.10, "ELEC_INTERRUPTORES", color)
    add_text(msp, "S3" if kind == "conmutado" else "S", x, y - 0.23, 0.12, "ELEC_INTERRUPTORES")


def sym_outlet(msp, x, y, kind, circuit):
    color = CIRCUIT_COLORS.get(circuit, COLORS["blue"])
    circle(msp, (x, y), 0.13, "ELEC_TOMACORRIENTES", color)
    line(msp, (x - 0.08, y), (x + 0.08, y), "ELEC_TOMACORRIENTES", color)
    if kind in ("tierra", "protegido", "especial", "cocina", "lavadora", "bomba"):
        line(msp, (x, y - 0.10), (x, y + 0.10), "ELEC_TOMACORRIENTES", color)
    if kind == "protegido":
        add_text(msp, "P", x + 0.22, y, 0.11, "ELEC_TOMACORRIENTES")
    if kind == "especial" or kind == "bomba":
        add_text(msp, "E", x + 0.22, y, 0.11, "ELEC_TOMACORRIENTES")


def sym_panel(msp, x, y, label):
    rect(msp, x - 0.22, y - 0.16, 0.44, 0.32, "ELEC_TABLEROS", COLORS["red"])
    add_text(msp, label, x, y, 0.12, "ELEC_TABLEROS")
...
def draw_legend(msp, x, y, title, circuits):
    rect(msp, x, y - 7.8, 7.2, 7.8, "ELEC_LEYENDA", COLORS["black"])
    ...
    for c in circuits:
        ...
        add_text(msp, f"{c['id']}: {c['uso']}", x + 0.9, yy, 0.13, "ELEC_LEYENDA", align=TextEntityAlignment.MIDDLE_LEFT)
        yy -= 0.28
